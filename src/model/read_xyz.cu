...
static std::vector<std::string> get_atom_symbols(std::string& filename_potential)
{
  ...
  if (tokens.size() >= 1 && tokens[0] == "MTP") {
    int sus2_species_count = -1;
    std::string potential_line;
    while (std::getline(input_potential, potential_line)) {
      std::vector<std::string> potential_tokens = get_tokens(potential_line);
      if (potential_tokens.size() >= 3 && potential_tokens[0] == "species_count" &&
          potential_tokens[1] == "=") {
        sus2_species_count = get_int_from_token(potential_tokens[2], __FILE__, __LINE__);
        break;
      }
    }
    if (sus2_species_count <= 0) {
      PRINT_INPUT_ERROR("SUS2 MTP potential file is missing a valid species_count.");
    }
    std::ifstream input_run("run.in");
    if (!input_run.is_open()) {
      PRINT_INPUT_ERROR("No run.in.");
    ...
    std::string line;
    while (std::getline(input_run, line)) {
      std::vector<std::string> run_tokens = get_tokens(line);
      if (run_tokens.size() >= 3 && run_tokens[0] == "potential" &&
          run_tokens[1] == filename_potential) {
        if (static_cast<int>(run_tokens.size()) < 2 + sus2_species_count) {
          PRINT_INPUT_ERROR(
            "SUS2 MTP potential line has fewer element symbols than species_count.");
        }
        return std::vector<std::string>(
          run_tokens.begin() + 2, run_tokens.begin() + 2 + sus2_species_count);
      }
    }
    PRINT_INPUT_ERROR(
      "SUS2 MTP potential files do not carry element symbols. Use e.g. "
      "'potential p1.1.mtp H C N I Pb' in run.in.");
  }
  if (tokens.size() < 3) {
