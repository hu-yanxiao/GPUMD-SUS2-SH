  ...
  const char** param, int num_param, const Box& box, const int number_of_atoms)
{
  ...
  const bool is_sus2_mtp = strcmp(potential_name, "MTP") == 0;
  if (is_sus2_mtp) {
    if (num_param < 2) {
      PRINT_INPUT_ERROR("SUS2 MTP potential should have a model file and optional species symbols.\n");
    }
  } else if (num_param != 2 && num_param != 3) {
    PRINT_INPUT_ERROR("potential should have 1 or 2 parameters.\n");
  }
  int num_types = is_sus2_mtp ? 0 : get_number_of_types(fid_potential);
